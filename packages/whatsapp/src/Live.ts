...
export const make = Effect.gen(function* () {
  ...
  const approvedTemplates: IWhatsAppGateway["approvedTemplates"] = Effect.gen(function* () {
      ...
      .execute(
        authed(
          HttpClientRequest.get(
            `${BASE_URL}/${businessId}/message_templates?status=APPROVED&fields=name,language,components`,
          ),
        ),
      )
    ...
    return listed.data.map(({ name, language, components }) => ({
      name,
      language,
      body: components?.find((component) => component.type === "BODY")?.text ?? "",
    }));
  });
